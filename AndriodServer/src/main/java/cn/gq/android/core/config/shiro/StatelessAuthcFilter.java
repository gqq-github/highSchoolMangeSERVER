...
import cn.gq.android.core.content.Constant;
...
public class StatelessAuthcFilter extends AccessControlFilter {
    ...
    // @Value("${system.config.filterExcludes}")
    ...
    private String[] defalutExcludeUrl = new String[]{
            "/websocket", "/register", "/logout", "/login", ".jpg", ".png", ".gif", ".css", ".js", ".jpeg", "/mongo" };

    private String[] extendExcludeUrl = new String[]{
            "/file"
    };
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        ...
        System.out.println(httpRequest.getIntHeader(AuthConstant.CLIENT_PARAM_ROLE));
        Integer role = httpRequest.getIntHeader(AuthConstant.CLIENT_PARAM_ROLE);
        if (token != null && role != null &&
            ...
            if (JwtTokenUtil.validateToken(authToken, AuthConstant.SECRETKET)) {
                //1、客户端传入的用户身份
                String userId = httpRequest.getHeader(AuthConstant.CLIENT_PARAM_USERID);

                //2、生成无状态Token
                StatelessToken statelessToken = new StatelessToken(userId, authToken);
                statelessToken.setRole(role);
                try {
                    //3、委托给Realm进行登录
